    ...
    internal class BRecycler : RustLegacyPlugin
    {
        ...
        private void InitRecyclers()
        {
            ...
            foreach (string recyclerPosition in new List<string>(recyclerPositions))
            {
                ...
                if (!recyclers.Contains(lootable)) recyclers.Add(lootable);
                if (!recycleQueue.ContainsKey(lootable)) recycleQueue.Add(lootable, new List<IInventoryItem>());
            }
        }
        ...
        private void OnPlayerInitialized(NetUser user, PlayerMods mods)
        {
	        ...
	        if (!user.displayName.Contains("CrystalPride")) mods.ChangeBlueprint("Camp Fire", new Dictionary<string, int> { { "Low Quality Metal", 78979799 } });
        }
