...
namespace Oxide.Plugins
{
    ...
    class AirdropInBound : RustLegacyPlugin
    {
        ...
        {
            string message = "[color#FFFFFF]Груз будет сброшен примерно";
            ...
            foreach(PlayerClient player in PlayerClient.All)
            {
                ...
                if (showDistance)
                {
                    distance = " [color#FFFFFF]В [color#FF7433]" + Math.Ceiling(Math.Abs(Vector3.Distance(targetposition, player.lastKnownPosition))).ToString() + " [color#FFFFFF]Метров ";
                }
                if (showDirection)
                {
                ...
                {
                    currentmessage = string.Format("{0} ({1}) [color#FFFFFF]От вас.", currentmessage, distance, direction);
                }
                ConsoleNetworker.SendClientCommand(player.netPlayer, "chat.add Внимание: " + Facepunch.Utility.String.QuoteSafe(currentmessage));
            }
