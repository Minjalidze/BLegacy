...
namespace Oxide.Plugins
{
    ...
    class ModRemover : RustLegacyPlugin
    {
        ...
        void mod_cmd(NetUser netuser, string command, string[] args)
        {
            ...
            if(economy.Balance < Price) { Broadcast.Message(netuser, $"[COLOR#FF7433]Вам не хватает {Price - economy.Balance}{Economy.CurrencySign} для заказа услуги.", _prefix); return; }
            if (inv.activeItem == null) { Broadcast.Message(netuser, "[COLOR#FF7433]Вы должны взять в руки оружие с которого нужно снять моды", _prefix); return; }
            ...
            if (inv.GetItem(inv.activeItem.slot, out item))
            {
                ...
                if (qty == 0)
                {
                    Broadcast.Message(netuser, "[COLOR#FF7433]Вам нужно зарядить [COLOR#FF7433]патроны в оружие!");
                    return;
                }
                ...
                Broadcast.Message(netuser, $"[COLOR#FF7433]Вы сняли все моды с  [COLOR#FF7433]\"{name}\"");
