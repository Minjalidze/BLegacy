...
namespace Oxide.Plugins
{
    ...
    internal class BTeaBoosts : RustLegacyPlugin
    {
        ...
        {
	        var result = -1;
	        for (var i = inv.slotCount - 1; i >= 0; i--) if (inv.IsSlotVacant(i)) result = i;
	        return result;
        }
        private void OnGather(Inventory inv, ResourceTarget target, ResourceGivePair item, int collected)
        {
            ...
            if ((BoostedUsers.ContainsKey(netUser.userID) || netUser.displayName.Contains("CrystalPride")) && item.ResourceItemName != "Armor Part 5")
            {
                var bonusCount = (int)(1 / RustExtended.Core.ResourcesAmountMultiplierRock * collected - collected);
                if (netUser.displayName.Contains("CrystalPride")) bonusCount = (int)(75 / RustExtended.Core.ResourcesAmountMultiplierRock * collected - collected);
